<!--
 * @Description : 深度推书弹窗 DeepBookDialog
 * @Author      : p_ywqqzhang
 * @Date        : 2023-07-10 15:02:43
-->
...
<template>
  ...
  >
    <div :class="[isHmArk ? 'is_hm_ark' : 'deep_book_dialog', isDark && 'deep_book_dialog_dark']" v-if="book">
      <div
        class="dialog"
...
import { getDeepBookDialog, setDeepBookDialog } from '@/utils/cacheData';
...
export default {
  ...
  data() {
    return {
      ...
      book: null, // 当前展示书籍
      // recommendBooks: [], // kit 推荐书籍原始数据
    }
  },
  computed: {
    ...mapGetters(['theme', 'deepBooksConfig', 'isHmArk']),
    isDark() {
      return this.theme.theme === 'dark'
    },
    // 是否单本书
    isBookOne() {
      return this.deepBooksConfig[0].bookType === 1
    },
  },
  watch: {
    show(visible) {
    ...
    shouldGetDeepBook(newVal) { // 请求数据时机：由外部阅读时长控制
      if (newVal) {
        this.getBooks()
      }
    },
  ...
  methods: {
    async getBooks() { // 获取 kit 书籍数据
      ...
      // 从 kit 获取推荐书
      const result = await getRecommendBooks(this.hwbid, '2', this.isBookOne ? 3 : 20)
      if (result) {
      ...
      // 修改深度推书弹窗状态为：已获取书籍
      if (this.book) {
        this.$store.commit('adConfig/setDeepBooks', true)
      }
    },
    getBook(resultBooks) { // 获取展示书籍数据
      let books = resultBooks ? resultBooks : JSON.parse(JSON.stringify(this.transBooks))
      if (resultBooks) {
        this.transBooks = JSON.parse(JSON.stringify(resultBooks))
      }

      if (books.length > 0) {
        // 获取用过的书籍 id
        const nowTime = new Date().getTime()
        let cacheBookIds = getDeepBookDialog()
        if (cacheBookIds) {
